...
inline bool operator==(const Point2d& lhs, const Point2d& rhs) {
  return(
    bg::get<0>(lhs) == bg::get<0>(rhs) && bg::get<1>(lhs) == bg::get<1>(rhs));
}
inline bool operator!=(const Point2d& lhs, const Point2d& rhs) {
  return !(lhs == rhs);
}
...
inline std::string print(const Point2d &p) {
  std::stringstream ss;
  ss << "Point2d: x: " << bg::get<0>(p) <<
    ", y: " << bg::get<1>(p) << std::endl;
  return ss.str();
}
...
struct Shape {
  ...
  virtual Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>
  ToArray() const = 0;
...
inline std::shared_ptr<Shape<G, T>> Shape<G, T>::Transform(
  ...
  boost::geometry::transform(
    obj_, obj_rel_translated, translate_rel_to_center);
  ...
  G obj_rotated;
  boost::geometry::transform(
    obj_rel_translated, obj_rotated, rotate);
