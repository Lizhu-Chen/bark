...
void odrSpiral(double s, double x0, double y0, double cDot, double kappa_start, double theta_start, double *x, double *y, double *t) {
  ...
  if (cDot == 0 && kappa_start == 0) {  // TODO(fortiss): check formula
    ...
    // Fresnel integrals
    double xxa = (kappa_start + cDot * s) / sqrt(M_PI * fabs(cDot));
    double ssa;
    double cca;

    fresnel(xxa, &ssa, &cca);

    double xxb = kappa_start / sqrt(M_PI * fabs(cDot));
    double ssb;
    double ccb;
    ...
    // Euler Spiral
    std::complex<double> cs1 = sqrt(M_PI / fabs(cDot)) * exp(ci * (theta_start - kappa_start * kappa_start / 2 / cDot));
    std::complex<double> cs2;
    if (cDot < 0)
      cs2 = -(cca - ccb) + ci * ssa - ci * ssb;
    else
      cs2 = (cca - ccb) + ci * ssa - ci * ssb;
  ...
  *x = cs.real();
  *y = cs.imag();
  *t = cDot * s * s * 0.5 + kappa_start * s + theta_start;
}
