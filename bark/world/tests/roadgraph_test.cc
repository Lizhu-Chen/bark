...
TEST(roadgraph, FindPath_test)
{
  ...
  std::vector<XodrLaneId> path = r.FindPath<TypeDrivingAndEdgeTypeLaneSuccessor>(l0,l4);
...
TEST(roadgraph, FindPath_in_unconnected_graph_test)
{
  ...
  r.AddLaneSuccessor(l0, l1);
  r.AddLaneSuccessor(l1, l2);
  // r.AddLaneSuccessor(l2, l3); REMOVING CONNECTION, thus not a valid path available
  r.AddLaneSuccessor(l3, l4);
...
TEST(roadgraph, find_drivable_path_in_unconnected_graph_test)
{
  ...
  std::vector<XodrLaneId> path = r.FindPath<TypeDrivingAndEdgeTypeLaneSuccessor>(l0,l3);
  ASSERT_EQ(0, path.size());
}
...
TEST(roadgraph, FindPath_along_neighbor_edges_test)
{
  ...
  std::vector<XodrLaneId> path = r.FindPath<TypeDrivingAndEdgeTypeLaneSuccessor>(l0, l2);
