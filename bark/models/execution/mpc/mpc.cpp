...
dynamic::Trajectory execution::ExecutionModelMpc::Execute(const float &new_world_time, const dynamic::Trajectory &trajectory,
                                                          const dynamic::DynamicModelPtr dynamic_model, const dynamic::State current_state) {
  ...
  dynamic::Trajectory desired_states(optimization_settings_.num_optimization_steps, static_cast<int>(StateDefinition::MIN_STATE_SIZE));
  desired_states.setZero();
  desired_states.col(StateDefinition::TIME_POSITION) = Eigen::ArrayXf::LinSpaced(
      optimization_settings_.num_optimization_steps, current_world_time,
      current_world_time + (optimization_settings_.num_optimization_steps - 1) * optimization_settings_.dt);

  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> weights_desired_states(optimization_settings_.num_optimization_steps,
                                                                              static_cast<int>(StateDefinition::MIN_STATE_SIZE));
  weights_desired_states.setZero();
  weights_desired_states.col(StateDefinition::TIME_POSITION) = Eigen::ArrayXf::LinSpaced(
      optimization_settings_.num_optimization_steps, current_world_time,
      current_world_time + (optimization_settings_.num_optimization_steps - 1) * optimization_settings_.dt).cast<double>();

  // get discrete states ready for optimization grid
  for (int i = 0; i < trajectory.rows(); ++i) {
    for (int j = 0; j < optimization_settings_.num_optimization_steps; j++) {
      if (trajectory(i, StateDefinition::TIME_POSITION) == current_world_time + j * optimization_settings_.dt) {
  ...
  // Overwrite first desired entry with current vehicle state no matter what planner wants
...
dynamic::Trajectory execution::ExecutionModelMpc::Optimize(std::vector<double *> parameter_block,
                                                           const dynamic::Trajectory &discrete_states,
                                                           const Eigen::Matrix<double, Eigen::Dynamic,
                                                           Eigen::Dynamic> weights_desired_states) {
  // objective function
  ...
  ceres::DynamicAutoDiffCostFunction<CostFunctor, static_cast<int>(StateDefinition::MIN_STATE_SIZE)> *cost_function = new ceres::DynamicAutoDiffCostFunction<CostFunctor, static_cast<int>(StateDefinition::MIN_STATE_SIZE)>(cost_functor);

  cost_function->AddParameterBlock(optimization_settings_.num_optimization_steps - 1);
  cost_function->AddParameterBlock(optimization_settings_.num_optimization_steps - 1);
  ...
  const_parameter_block = reinterpret_cast<const double *const *>(&parameter_block[0]);

  // call kinematic model one last time to get optimized trajectory
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> traj = KinematicModel<double>(const_parameter_block, discrete_states, optimization_settings_);
