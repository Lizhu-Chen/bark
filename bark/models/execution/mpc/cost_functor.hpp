...
inline Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> KinematicModel(
  ...
  for (int i = 1; i < optimization_settings.num_optimization_steps; i++) {
    ...
    traj.block(i, 0, 1, 5) = traj.block(i - 1, 0, 1, 5) + T(optimization_settings.dt) * f_dot;
  }
  return traj;
}
...
struct CostFunctor {
  ...
  bool operator()(T const *const *parameters, T *residual) {
    // initialize
    T cost = T(0.0);
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> traj = KinematicModel(parameters, desired_discrete_states_, optimization_settings_);
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> desired_discrete_states = desired_discrete_states_.cast<T>();
    ...
    for (int i = 0; i < optimization_settings_.num_optimization_steps - 1; i++) {
      ...
      xDiff = traj(i, StateDefinition::X_POSITION) - desired_discrete_states(i, StateDefinition::X_POSITION);
      yDiff = traj(i, StateDefinition::Y_POSITION) - desired_discrete_states(i, StateDefinition::Y_POSITION);
      thetaDiff = traj(i, StateDefinition::THETA_POSITION) - desired_discrete_states(i, StateDefinition::THETA_POSITION);
      velDiff = traj(i, StateDefinition::VEL_POSITION) - desired_discrete_states(i, StateDefinition::VEL_POSITION);
    ...
    residual[0] = cost / (weights.sum() + (weights_acc + weights_delta) * T(optimization_settings_.num_optimization_steps));
    return true;
  }
  ...
  bool set_optimization_settings(const OptimizationSettings &optimization_settings) {
    optimization_settings_ = optimization_settings;
    return true;
  }

  bool set_weights(const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> weights) {
    weights_ = weights;
    return true;
  }
